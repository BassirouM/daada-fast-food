...
const AVATAR_COLORS = ['#FF6B00', '#CC5500', '#E07B00', '#D44F00', '#FF8C3A', '#B84700']

// ─── Helpers ──────────────────────────────────────────────────────────────────

function makeAvatarSvg(name: string): string {
  const color = AVATAR_COLORS[name.charCodeAt(0) % AVATAR_COLORS.length] ?? '#FF6B00'
  const initials = name
    .trim()
    .split(/\s+/)
    ...
    .slice(0, 2)
    .join('')
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" viewBox="0 0 80 80"><circle cx="40" cy="40" r="40" fill="${color}"/><text x="40" y="40" dy="0.35em" text-anchor="middle" fill="white" font-family="system-ui,sans-serif" font-size="28" font-weight="700">${initials}</text></svg>`
...
function AuthPageInner() {
  ...
  const handleRegister = async () => {
    ...
    // Upload avatar if SVG was generated
    if (avatarSvg) {
      await fetch('/api/users/profile', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ avatar_url: avatarSvg }),
      })
    }
  ...
  // Avatar preview from name
  useEffect(() => {
    if (nom.trim().length >= 2) {
      setAvatarSvg(makeAvatarSvg(nom.trim()))
    } else {
      setAvatarSvg('')
    }
  }, [nom])

  // File input for avatar photo
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (!file) return
    const reader = new FileReader()
    reader.onload = () => setAvatarSvg(reader.result as string)
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <div
                    style={{
                      ...
                      borderRadius: '50%',
                      overflow: 'hidden',
                      background: avatarSvg ? 'none' : 'var(--bg-elevated)',
                      border: '2px solid var(--border)',
                      flexShrink: 0,
                      display: 'flex',
                      alignItems: 'center',
                  ...
                  >
                    {avatarSvg ? (
                      ...
                      <img src={avatarSvg} alt="avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    ) : '👤'}
                  </div>
                  <div>
